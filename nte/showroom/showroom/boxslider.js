    ...
    $.fn.bxSlider = function(options){
		...
		return this.each(function(){
			...
			if(o.alignment == 'horizontal'){
				...
				$this.parent().css({'position':'relative', 'overflow':'hidden',	'width':w-o.margin,'height':'540px'});//FJ
			...
			if(o.pager && !o.ticker){
				$this.parent().after('<div class="bx-pager"></div>');
				if(!o.pager_short){
					var $a;
					$kids.each(function(index){
						$a = $('<a href="#">'+(index+1)+'</a>');
						$this.parent().siblings('.bx-pager').append($a);
						$a.click(function(){
							is_working = false;
							tick_play = false;
							$this.stop();
					...
					$this.parent().siblings('.bx-pager').append();
				}
				set_active(1);
			}

			if(o.controls && !o.ticker){
				if(o.next_image != '' || o.prev_image != ''){$this.parent().after('<a class="bx-prev" href="#"><img src="'+o.prev_image+'" /></a><a class="bx-next" href="#"><img src="'+o.next_image+'" /></a>');}
				else{$this.parent().after('<a class="bx-prev" href="#">'+o.prev_text+'</a><a class="bx-next" href="#">'+o.next_text+'</a>');}
			...
			if(o.auto && !o.ticker){
				...
				if(o.auto_controls){
					$this.parent().after('<div class="bx-auto"><a class="auto_link" href="#">'+o.stop_text+'</a></div>');
					$this.parent().siblings('.bx-auto').find('a').click(function(){
			...
			function set_active(num){
				if(o.pager && !o.pager_short){$this.parent().siblings('.bx-pager').find('a').removeClass('active').eq(num-1).addClass('active');}
				else if(o.pager_short){$this.parent().siblings('.bx-pager').html(num+o.pager_short_separator+$kids.length);}
			}
